...
export const HOST_PAGE_JS = [
  ...
  "}",
  "",
  "// Watch ids restart in every fresh document, so a result minted for the",
  "// document before a reload must never reach the new one: same id,",
  "// different watch. Raised on every ready and every new version, like",
  "// the production host's document generation.",
  "let generation = 0;",
  "",
  "async function runToolCall(id, tool, params) {",
  "  const calledIn = generation;",
  ...
  "  // Like the production host: only the service's own failure kinds reach",
  "  // the app's onError; anything else in the field is dropped.",
  "  const kind = payload && payload.error ? payload.error.kind : undefined;",
  '  if (kind === "timeout" || kind === "unavailable" || kind === "busy" || kind === "rate_limited") {',
  "    failure.kind = kind;",
  "  }",
  '  postToApp({ protocol: PROTOCOL, type: "result", id: id, error: failure });',
  "}",
  "",
